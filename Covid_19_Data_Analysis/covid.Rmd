...
### Import the various csv files into r variables
...
### Lowest Rate
```{r, lowest_case_rate, warning=FALSE}
us_state_totals %>%
  slice_min(deaths_per_thou, n = 10)
```

### Model deaths vs cases
```{r, model_cases_deaths, warning=FALSE}
mod <- lm(deaths_per_thou ~ cases_per_thou, data = us_state_totals)
summary(mod)
```

### Prediction vs actual on plot
```{r, plot_prediction, warning=FALSE}
us_state_totals %>% mutate(pred = predict(mod))
```

### Create prediction table
```{r, create_prediction_table, warning=FALSE}
us_state_totals_predictions <- us_state_totals %>% 
  mutate(pred = predict(mod))
```

### Plot prediction vs actual
```{r, plot_prediction_table, warning=FALSE}
