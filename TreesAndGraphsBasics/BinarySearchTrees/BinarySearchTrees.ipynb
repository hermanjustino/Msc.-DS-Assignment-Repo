...
class Node: 
    ...
    def get_rightmost_descendant(self):
        if self.right != None:
            return self.right.get_rightmost_descendant()
        else:
            return self
...
# The worst possible case would be ascending order
...
# If s1 is the root of a tree, if the tree is non-empty, it must be min or max depending on ascending or descending order
